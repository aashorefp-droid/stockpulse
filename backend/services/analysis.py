...
def calc_trade_levels(daily_df: pd.DataFrame, verdict: str, current_price: float) -> dict:
    ...
    avg_daily_move = atr * 0.6 if atr > 0 else current_price * 0.01

    entry = round(current_price, 2)

    if verdict in ("BULLISH", "LEAN BULLISH"):
        stop_raw = recent_lo - atr * atr_mult
        risk = max(entry - stop_raw, atr * 0.8, entry * 0.015)
        stop = round(entry - risk, 2)
        t1   = round(entry + risk * 2, 2)
        t2   = round(entry + risk * 3, 2)
        rr1  = round((t1 - entry) / risk, 2)
        rr2  = round((t2 - entry) / risk, 2)
        t1_days = max(1, round((t1 - entry) / avg_daily_move)) if avg_daily_move > 0 else 5
        t2_days = max(1, round((t2 - entry) / avg_daily_move)) if avg_daily_move > 0 else 10
    elif verdict in ("BEARISH", "LEAN BEARISH"):
        stop_raw = recent_hi + atr * atr_mult
        risk = max(stop_raw - entry, atr * 0.8, entry * 0.015)
        stop = round(entry + risk, 2)
        t1   = round(entry - risk * 2, 2)
        t2   = round(entry - risk * 3, 2)
        rr1  = round((entry - t1) / risk, 2)
        rr2  = round((entry - t2) / risk, 2)
        t1_days = max(1, round((entry - t1) / avg_daily_move)) if avg_daily_move > 0 else 5
        t2_days = max(1, round((entry - t2) / avg_daily_move)) if avg_daily_move > 0 else 10
    else:
        # Neutral setups: derive adaptive direction from 5-day momentum or MA to ensure targets are always present
        try:
            p_5d = float(daily_df[close_col].iloc[-5]) if len(daily_df) >= 5 else current_price
            lean_bullish = current_price >= p_5d
        except Exception:
            lean_bullish = True

        risk = max(atr * 1.0, entry * 0.02)
        if lean_bullish:
            stop = round(entry - risk, 2)
            t1   = round(entry + risk * 2, 2)
            t2   = round(entry + risk * 3, 2)
...
def get_fundamentals(ticker: str) -> dict:
    try:
        ...
        cur_px = _s("currentPrice") or _s("regularMarketPrice") or 0
        tgt_px = _s("targetMeanPrice") or _s("targetMedianPrice")
        tgt_upside = round(((tgt_px - cur_px) / cur_px) * 100, 1) if (tgt_px and cur_px > 0) else None
        return {
            ...
            "target_price":    round(float(tgt_px), 2) if tgt_px else None,
            "analyst_target":  round(float(tgt_px), 2) if tgt_px else None,
            "target_1y":       round(float(tgt_px), 2) if tgt_px else None,
...
def analyze_institutional_control(weekly_df, lookback=26):
    ...
    try:
        ...
        vah = wk_low_26 + (wk_range * 0.70)
        val = wk_low_26 + (wk_range * 0.30)
        ...
        try:
            ...
            for i in range(len(df_wk) - 2, -1, -1):
                ...
                if not in_same_phase:
                    ...
                    days_in_phase = (current_date - transition_date).days + 7
                    break
                if i == 0:
